...
public class player_attack : MonoBehaviour
{
    ...
    void StartCharge()
    {
        ...
        // ⭐ 溜め開始ログ
        Debug.Log("[Charge] StartCharge() 溜め開始");
    ...
    void ReleaseCharge()
    {
        ...
        // ⭐ 溜め解除ログ
        Debug.Log("[Charge] ReleaseCharge() 溜め解除 → 攻撃発生");

        // RootMotion 解除（必要なら）
        if (animator != null)
            animator.applyRootMotion = false;

        basePower = chargeAttackPower;

        if (attackHit)
        {
            Debug.Log("[Charge] Hitbox Enable（チャージ攻撃）");
            // RootMotion 使うならここでON
            //if (animator != null)
